...
{
private:
    chain<int> *table; // 链表数组
    int D;             // 除数
public:
    hashChain(int d)
    {
        D = d;
        table = new chain<int>[D];
    }
...
void hashChain::hashInsert(int x)
{
    int hash_value = x % D;
    if (table[hash_value].indexOf(x) != -1) // 已经存在该元素
        cout << "Existed" << endl;
    else
        table[hash_value].push_back(x);
}

void hashChain::hashSearch(int x)
{
    int hash_value = x % D;
...
void hashChain::hashDelete(int x)
{
    int hash_value = x % D;
    if (table[hash_value].indexOf(x) == -1)
