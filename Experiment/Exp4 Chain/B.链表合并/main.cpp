...
void chain<T>::sort()
{
    ...
    chain<T>::iterator it = begin();
    for (int j = 0; it != end(); it++, j++)
        arr[j] = *it;                  // 链表数据域转入数组
    for (int i = 1; i < listSize; i++) // 插入排序
...
int value_calculate(chain<int> &c)
{
    int value = 0;
    chain<int>::iterator cp = c.begin();
    for (int i = 0; cp != c.end(); i++, cp++)
        value += i ^ (*cp); // 计算异或和
    return value;
}
...
int main()
{
    ...
    chain<int>::iterator sump = sum.begin();
    while (ap != NULL || bp != NULL)
    {
        if (ap == NULL) // 当a到末尾了
        {
            sum.push_back(*bp); // 插入bp的值
            bp++;               // 移动bp
        }
        else if (bp == NULL)
        {
    ...
    cout << value_calculate(a) << endl
         << value_calculate(b) << endl
         << value_calculate(sum) << endl;
    return 0;
}
