...
template <class T>
chain &chain<T>::operator=(const chain<T> &theList)
{
    if (this != &theList)
    {
        // 删除旧链表
...
int main()
{
    int n, q;
    cin >> n >> q;
    chain<int> chain_list;
    for (int i = 0; i < n; i++)
    {
        int value;
        cin >> value;
        chain_list.push_back(value);
    }
    for (int i = 0; i < q; i++)
    {
        int operation;
        ...
        case 2:
        {
            int val;
            cin >> val;
            int index = chain_list.indexOf(val);
            if (index != -1)
                chain_list.erase(index);
            else // 说明没找到
                cout << "-1" << endl;
            break;
        }
        ...
        case 4:
        {
            int val;
            cin >> val;
            cout << chain_list.indexOf(val) << endl;
            break;
        }
        case 5:
        {
            chain<int>::iterator it = chain_list.begin(); // 创建迭代器
            int sum = 0;
            for (int i = 0; it != chain_list.end(); i++, it++)
                sum += i ^ (*it); // 计算异或和
            cout << sum << endl;
            break;
        }
